...
try {
    ...
    if ($round['status'] !== 'ranking') {
        http_response_code(400);
        echo json_encode(['error' => 'Not in ranking phase']);
        exit;
    }

    if ((int)$player['id'] !== (int)$round['target_player_id']) {
        http_response_code(403);
        echo json_encode(['error' => 'Only the Target Player can submit a ranking']);
        exit;
    }
    ...
    $db->prepare(
        "UPDATE rounds SET target_ranking = ?, status = 'guessing', ranking_deadline = NULL WHERE id = ?"
    )->execute([json_encode($ranking), $round['id']]);

    bump_version($db, (int)$game['id']);
    insert_system_chat($db, $player['lobby_id'], "Rankings submitted! Discuss and arrange the cards.");
